    ...
    if request.method == 'POST':
        ...
        if sql != "UPDATE Home SET ":
            ...
            if res == 1:
                ...
                return jsonify({"message": "Error updating home "}), 500
        else:
        ...
        if res == 1:
            ...
            return jsonify({"message": "Error deleting home "}), 500

    elif request.method == 'GET':
        # https://werkzeug.palletsprojects.com/en/2.2.x/datastructures/#werkzeug.datastructures.MultiDict.get
        ...
        try:
            ...
            return jsonify(message="Unable to get data " + info), 500
...
@ app.route('/sensor', methods=['POST', 'DELETE', 'GET'])
def sensor():
    if request.method == 'POST':
        # get the data from the body
        data = request.get_json()
        ...
        if res == 1:
            ...
            return jsonify({"message": "Error creating sensor "}), 500
...
def list_all_data():
    if request.method == 'GET':
        # get all homes from a homeowner
        sql = "SELECT * FROM Home_Owner ho left join Home h on ho.name = h.home_owner left join Sensor s on h.name = s.home_name"
        try:
            result = select_from_db(sql)
            return jsonify(result), 200
        except:
    ...
    if request.method == 'POST':
        ...
        info = json.dumps(data)
        try:
            validate(instance=data, schema=bulkImport_schema)
            return jsonify({"message": "Added Successgully" + info}), 200
        except:
            return jsonify({"message": "Invalid data"}), 400
